...
Notes:
- After populating green, rest of it uses green information, which was 
  calculated by some other threads in some other SM unit or block so that we access not modified green values.(race condition)
*/
...
void PopulateParallel3(Image<T,idxT>& image){
    gpu_selector gpu;                
