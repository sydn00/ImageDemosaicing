/*Demosaicing Kernel 3.2

6x8 Image Demonstration for G R, B G Bayer CFA pattern (- = data, + = threadIdx ) 
event = Populates all, each thread populates green first then red and blue in 2x2 data
...
Notes:
- Nd_range kernel is used.
- Barrier is placed between populating green and rest. 
- unlike parallel_for nd_range doesn't allow more than available thread, strided access pattern is used to accomplish this.
- Total thread count and thread group size have to be modified to make algorithm compact for any image aspect ratio. (not done) 
*/
...
void PopulateParallel3_2(Image<T,idxT>& image){
    gpu_selector gpu;                
    ...
    
    {
        ...
        */
        ...
        event PopulateAllEvent = Q.submit([&](handler& h){
            accessor<T,3> imgAcc(imgBuffer,h,read_write);
           

            h.parallel_for(nd_range<2>{range<2>(640,640),range<2>(16,16)}, [=](nd_item<2> item){
    
                //strided loop, each thread computes more than one pixel value
                idxT stridex = item.get_global_range(0);
                idxT stridey = item.get_global_range(1);
                for(idxT ix=2*item.get_global_id(0)+2;ix<height;ix+=2*stridex){
                    for(idxT iy=2*item.get_global_id(1)+2;iy<width;iy+=2*stridey){

                        idxT i,j;

                        LT A1,A2,A3,A4,A5,A6,A7,A8,A9;
                        LT G1,G2,G3,G4,G5,G6,G7,G8,G9;
                        LT alpha,beta;
                        //Populating Green
